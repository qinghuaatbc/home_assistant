...
async function create(config, services) {
    const logger = new common_1.Logger('WeatherStation');
    const cfg = {
        name: 'Weather Station',
        host: 'localhost',
        port: 8080,
    ...
    async function updateStates(data) {
        for (const eid of entityIds) {
            ...
            if (eid.endsWith('_temperature'))
                value = String(data.temperature ?? 'unavailable');
            else if (eid.endsWith('_humidity'))
                value = String(data.humidity ?? 'unavailable');
            else if (eid.endsWith('_wind_speed'))
                value = String(data.wind_speed ?? 'unavailable');
            else if (eid.endsWith('_wind_direction'))
                value = data.wind_direction ?? 'unavailable';
            else if (eid.endsWith('_pressure'))
                value = String(data.pressure ?? 'unavailable');
            else if (eid.endsWith('_rain'))
                value = String(data.rain ?? 'unavailable');
            if (value !== null) {
    ...
    return {
        ...
        async on_config_change(newConfig) {
